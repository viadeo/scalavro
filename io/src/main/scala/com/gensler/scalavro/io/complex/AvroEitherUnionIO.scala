...
private[scalavro] case class AvroEitherUnionIO[U <: Union.not[_]: TypeTag, T <: Either[_, _]: TypeTag](
  ...
  def writeHelper[X <: T: TypeTag, A: TypeTag, B: TypeTag](obj: X, encoder: BinaryEncoder) =
    obj match {
      case Left(value)  => leftAvroType.asInstanceOf[AvroType[A]].write(value.asInstanceOf[A], encoder)
      case Right(value) => rightAvroType.asInstanceOf[AvroType[B]].write(value.asInstanceOf[B], encoder)
    }
  ...
  def readHelper[A: TypeTag, B: TypeTag](decoder: BinaryDecoder) = {
    ...
    if (index == 0) Left(leftAvroType.read(decoder).get.asInstanceOf[A])
    else if (index == 1) Right(rightAvroType.read(decoder).get.asInstanceOf[B])
